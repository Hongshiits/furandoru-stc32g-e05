...
#define FIRST_ORDER_LPF_TWO_PI     (6.28318530718f)

//-------------------------------------------------------------------------------------------------------------------
// First order low pass filter.
// y(k) = y(k - 1) + alpha * (x(k) - y(k - 1)), alpha range is 0.0f to 1.0f.
//-------------------------------------------------------------------------------------------------------------------
void first_order_lpf_init (first_order_lpf_struct *filter, float alpha, float init_value)
{
    zf_assert(filter != NULL);
    do
    {
        if(NULL == filter)
        {
        ...
        filter->initialized = ZF_TRUE;
    }while(0);
}

//-------------------------------------------------------------------------------------------------------------------
// Initialize filter by cutoff frequency and sample period.
//-------------------------------------------------------------------------------------------------------------------
void first_order_lpf_init_by_cutoff (first_order_lpf_struct *filter, float cutoff_freq_hz, float sample_period_s, float init_value)
{
    float alpha = 1.0f;

    if((0.0f < cutoff_freq_hz) && (0.0f < sample_period_s))
    {
        float rc = 1.0f / (FIRST_ORDER_LPF_TWO_PI * cutoff_freq_hz);
        alpha = sample_period_s / (rc + sample_period_s);
    }

    first_order_lpf_init(filter, alpha, init_value);
}

//-------------------------------------------------------------------------------------------------------------------
// Reset filter output.
//-------------------------------------------------------------------------------------------------------------------
void first_order_lpf_reset (first_order_lpf_struct *filter, float value)
{
    zf_assert(filter != NULL);
    do
    {
...
//-------------------------------------------------------------------------------------------------------------------
// Update filter once and return filtered value.
//-------------------------------------------------------------------------------------------------------------------
float first_order_lpf_update (first_order_lpf_struct *filter, float input)
{
    float result = input;
